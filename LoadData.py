...
def load_data():
	...
	for dataset in ['train']:
		file=open(os.path.join(WORKING_PATH,"text_data1/",dataset+".txt"),"r")
		for line in file:
			content=eval(line)
			image=content[0]
			sentence=content[1]
			group=content[2]
			if os.path.isfile(os.path.join(WORKING_PATH1,"image_data/",image+".jpg")):
...
# load word index
def load_word_index():
	with open(os.path.join(WORKING_PATH,"text_embedding1/vocabs.txt"), 'r',encoding='utf-8') as f:
		for line in f:
...
class my_data_set(Dataset):
	def __init__(self, data):
		...
		for id in data.keys():
			self.data[id]["image_path"] = os.path.join(WORKING_PATH1,"image_data/",str(id)+".jpg")
	...
	def __image_feature_loader(self,id):
		image_feature = np.load(os.path.join(WORKING_PATH1,"image_feature_data",str(id)+".npy"))
